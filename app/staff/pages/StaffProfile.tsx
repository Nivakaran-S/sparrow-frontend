...
const StaffProfile = ({ setActiveTab }: TrackShipmentsProps) => {
  ...
  const fetchUserData = async () => {
    try {
      ...
      if (authResponse.ok) {
        ...
        const customerResponse = await fetch(`${API_BASE_URL}/api/users/staff/${authData.id}`, {
          credentials: 'include',
        });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/users/staff/${userId}`, {
        method: 'PUT',
        credentials: 'include',
        headers: {
        ...
        <div className="flex items-center justify-between mb-8 pb-6 border-b border-gray-700">
          ...
          {!isEditing && (
            <button
              ...
              className="px-6 cursor-pointer py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition-colors"
            >
              Edit Profile
            </button>
          )}
