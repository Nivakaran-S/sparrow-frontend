...
const DriverProfile = () => {
  ...
  const fetchUserData = async () => {
    try {
      ...
      if (authResponse.ok) {
        ...
        const driverResponse = await fetch(`${API_BASE_URL}/api/users/driver/${authData.id}`, {
          credentials: 'include',
        });
        
        if (driverResponse.ok) {
          const driverData = await driverResponse.json();
          const driver = driverData[0]; // Customer data is first element
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/users/driver/${userId}`, {
        method: 'PUT',
        credentials: 'include',
        headers: {
  ...
  return (
    <div className="text-white ml-[5vw] pt-[3vh]">
      <div className="mb-8">
